...
#else
		trace("tlsf_add_pool: Memory size must be between %u and %u bytes.\n",
			(unsigned int)(pool_overhead + block_size_min),
...
int test_ffs_fls()
{
	...
	if (rv)
	{
		trace("test_ffs_fls: %x ffs/fls tests failed.\n", rv);
	}
	return rv;
}
#endif
...
tlsf_t tlsf_create(void* mem)
{
	...
	{
		trace("tlsf_create: Memory must be aligned to %u bytes.\n",
			(unsigned int)ALIGN_SIZE);
		return 0;
	}
